...
public class EmailRandomiser implements IRandomiserFunctionality
{
    ...
    public void setRandomiserInstance(RandomiserInstance ri)
    {
        ...
        try
        {
            vTopDomains = utils.readFile("resources" + File.separator +"TLDs.txt");
            vFirstNames = utils.readFile("resources" + File.separator + "en_firstnames.txt");
            vLastNames = utils.readFile("resources" + File.separator +"en_lastnames.txt");
            vProviders = utils.readFile("resources" + File.separator + "emailproviders.txt");
        }
    ...
    public Object generate()
    {
        ...
        fname = vFirstNames.elementAt(i2).trim();
        if(fname.contains(",m") || fname.contains(",f") ){
	        len   = fname.length();
